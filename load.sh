...
process_table() {
  ...
  if [[ $(wc -l < tables/${table}.csv) -ge 500 ]]; then
    ...
    bq load --skip_leading_rows 1 --null_marker NULL --allow_quoted_newlines --autodetect --schema_update_option ALLOW_FIELD_ADDITION phrg.${table} heads/${table}.csv
    ...
    bq load --skip_leading_rows 1 --null_marker NULL --allow_quoted_newlines phrg.${table} tables/${table}.csv schemas/${table}.json
  else
    if [[ $(wc -l < tables/${table}.csv) -ge 2 ]]; then
      echo "${table} - Loading full table"
      bq load --skip_leading_rows 1 --null_marker NULL --autodetect --schema_update_option ALLOW_FIELD_ADDITION --allow_quoted_newlines phrg.${table} tables/${table}.csv
    else 
